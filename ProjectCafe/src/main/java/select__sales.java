...
/**
 * Servlet implementation class select__sales
 */
@WebServlet("/select__sales")
public class select__sales extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public select__sales() {
        super();
		...
		request.setCharacterEncoding("utf-8");
	      ...
	      try {
	         ...
	         while (rs.next()) {
	            if (!strReturn.equals("")) 
	            strReturn += ";";
	            strReturn += rs.getInt("code") + "," + rs.getString("mobile") + "," + rs.getInt("menu_code")
	            			 + "," + rs.getInt("qty") + "," + rs.getInt("total");
	         }
